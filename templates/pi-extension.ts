...
type StatsEntry = { event: string; path: string; tokensSaved: number; ts: number };
...
const MAX_BYTES = parseInt(process.env.READ_ONCE_MAX_BYTES || String(1024 * 1024), 10);
...
const EXCLUDED = [
  ".git/", "node_modules/", ".DS_Store", "/proc/", "/sys/",
  "/dev/", ".sock", ".lock", ".pid",
...
function bashReadsFile(cmd: string): { path: string; offset?: number; limit?: number } | null {
  ...
  const readBins = new Set(["cat", "head", "tail", "less", "more", "bat", "highlight", "nl"]);
  if (!readBins.has(bin)) return null;

  const args = c.slice(bin.length).trim();
  if (!args || args.startsWith("-")) {
    const parts = args.split(/\s+/);
    const fileArg = parts.find(p => p && !p.startsWith("-") && !p.includes("="));
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (t === "-n" || t === "--lines" || t === "-") {
      const v = tokens[i + 1];
      if (/^\d+$/.test(v)) { limit = parseInt(v, 10); i++; }
      continue;
    }
  ...
  const result: { path: string; offset?: number; limit?: number } = { path: files[0] };
...
export default function (pi: ExtensionAPI) {
  ...
  function checkFile(filePath: string, cwd: string, offset?: number, limit?: number): { block: boolean; reason: string } | undefined {
    ...
    const cacheKey = hasRange ? `${cleanPath}:${offset}:${limit ?? 0}` : cleanPath;
    const now = Date.now();
    ...
    if (entry && !isExpired(entry, now)) {
      ...
      try {
        ...
        if (st.mtimeMs !== entry.mtime) {
          ...
          cache.set(cacheKey, { mtime: st.mtimeMs, ts: now, attempts: 0, lastAttempt: now, hash });
          stats.push({ event: "changed", path: cleanPath, tokensSaved: 0, ts: now });
          return undefined;
        }
      } catch {
      ...
      if (HASH_MODE && entry.hash) {
        ...
        if (currentHash && currentHash !== entry.hash) {
          ...
          cache.set(cacheKey, { mtime: entry.mtime, ts: now, attempts: 0, lastAttempt: now, hash: currentHash });
          stats.push({ event: "changed", path: cleanPath, tokensSaved: 0, ts: now });
          return undefined;
        }
      }
      ...
      const attempts = (now - entry.lastAttempt <= DECAY_S * 1000) ? entry.attempts + 1 : 1;
      ...
      if (AUTO_ALLOW > 0 && attempts >= AUTO_ALLOW) {
        ...
        cache.set(cacheKey, { mtime: entry.mtime, ts: now, attempts: 0, lastAttempt: now, hash: entry.hash });
        stats.push({ event: "auto_allow", path: cleanPath, tokensSaved: 0, ts: now });
        return undefined;
      }

      const minutesAgo = Math.floor((now - entry.ts) / 60000);
      const ttlMin = Math.floor(ttl / 60);
      const label = hasRange ? `${basename(cleanPath)}:${offset}:${limit ?? 0}` : basename(cleanPath);
  ...
  pi.registerTool({
    ...
    description: "Clear the read-once file cache, allowing re-reads of previously cached files. Use when you need to force a fresh read of a file that was blocked.",
    parameters: Type.Object({
      file_path: Type.Optional(Type.String({ description: "Specific file to clear from cache. Omit to clear all." })),
    }),
  ...
  pi.registerTool({
    ...
    description: "Show read-once cache statistics for this session: hits, misses, tokens saved, and top re-read files.",
    parameters: Type.Object({}),
    async execute() {
      ...
      if (tokensSaved > 0) {
        lines.push(`  Est. cost saved:   $${(tokensSaved * 3 / 1_000_000).toFixed(4)} (Sonnet) / $${(tokensSaved * 15 / 1_000_000).toFixed(4)} (Opus)`);
      }
      if (topFiles) {
  ...
  debug(`loaded (mode=${MODE}, ttl=${TTL_S}s, hash=${HASH_MODE ? "on" : "off"}, auto_allow=${AUTO_ALLOW})`);
}
